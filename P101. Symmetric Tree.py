# ToDo:
# probly change thr original tree!!!!
"""
101. Symmetric Tree
Easy
...
"""
Success
Runtime: 40 ms, faster than 50.61% of Python3 online submissions for Symmetric Tree.
		...
		if type(tree_node) != TreeNode:
			...
			for i in range(len(treenode_list)):
				if treenode_list[i] != None:
					treenode_list[i] = treenode_list[i].val
			return treenode_list
	original_list = TreeNode2List(root) #or will change oringinal tree!!!
	mirror_tree = mirrorTree(root)
	if not mirror_tree:
		return True
	else:
